...
#include "../RTPack/source/stb/stb_image_write.h"
#include "../RTPack/source/stb/stb_image.h"
...
bool DecompressAndSave(std::string fName) {
	...
	if (size >= C_RTFILE_PACKAGE_HEADER_BYTE_SIZE && IsAPackedFile(pInput))
	{
		...
		auto decompressed = DecompressRTPackToMemory(pInput, &real_size);
		SAFE_DELETE_ARRAY(pInput);
		if (!decompressed) {
			LogMsg("For some reason can't decompress the file");
			PressAnyKey();
			return true;
		}
		auto decomp = GetFileNameWithoutExtension(fName) + "_deco.rttex";
		FILE* fp = fopen(decomp.c_str(), "wb");
		fwrite(decompressed, real_size, 1, fp);
		SAFE_DELETE_ARRAY(decompressed);
		fclose(fp);
		LogMsg("Decompressed. Saved as %s (%.1f kb, %.2f times bigger)", decomp.c_str() ,real_size / 1024.0f, float(real_size) / float(size));
	}
	else {
		SAFE_DELETE_ARRAY(pInput);
		LogMsg("File is not compressed or invalid rttex file.");
		PressAnyKey();
	}
	return true;
}
bool Unpack(string fName) {
	uint32_t size = 0;
	byte* pInput = LoadFileIntoMemoryBasic(fName, &size, false, false); //the basic means don't try to decompress it

	if (!pInput) {
		LogMsg("Unpacking: Can't read the file %s\n", fName.c_str());
		PressAnyKey();
		return true;
	}
	...
	if (size >= C_RTFILE_PACKAGE_HEADER_BYTE_SIZE && IsAPackedFile(pInput))
	{
		...
		if (!decompressed) {
			LogMsg("Could not decompress file for some reason...");
			PressAnyKey();
			return true;
		}
		size = real_size;
		pInput = decompressed;
		LogMsg("Decompressed (%.1f kb, %.0fx bigger)", real_size / 1024.0f, float(real_size) / float(size));
	}
	else
		LogMsg("Not compressed, skipping it");

	if (!pInput || size < sizeof(rttex_header)) {
		LogMsg("Something wen't wrong, maybe %s is not a rttex file?", fName.c_str());
	...
	rttex_header* header = (rttex_header*)pInput;
	std::string type = header->rtFileHeader.fileTypeID;
	if (type != "RTTXTR") {
		LogMsg("Unsupported rtheader magic: %s, only RTTXTR is supported.", type.c_str());
		SAFE_DELETE_ARRAY(pInput);
		PressAnyKey();
		return true;
	}
	else if (header->format != GL_UNSIGNED_BYTE) {
		LogMsg("Unsupported format: %d, only GL_UNSIGNED_BYTE is supported.", header->format);
		SAFE_DELETE_ARRAY(pInput);
		PressAnyKey();
		return true;
	}
	if (GetApp()->header_info)
		LogMsg("Size: %d x %d\tAlpha: %s\tMipmap Count: %d\tOriginal Size: %d x %d", header->height, header->width, header->bUsesAlpha ? "yes" : "no", header->mipmapCount, header->originalHeight, header->originalWidth);

	uint8_t* texture = (uint8_t*)(pInput + sizeof(rttex_header) + sizeof(rttex_mip_header) * header->mipmapCount);
	auto basename = GetFileNameWithoutExtension(fName);
	auto new_name = basename + ".png";

	if (FileExists(new_name)) {
		for (int i = 2; i < 30; i++) {
			new_name = basename + "_" + std::to_string(i) + ".png";
			if (!FileExists(new_name))
				break;
		}
	}
	stbi_set_flip_vertically_on_load(true);
	stbi_write_png(new_name.c_str(), header->width, header->height, header->bUsesAlpha ? 4 : 3, texture, header->width * (header->bUsesAlpha ? 4 : 3));
	LogMsg("Saved as %s", new_name.c_str());
	SAFE_DELETE_ARRAY(pInput);
	return true;
...
bool CompressFile(string fName)
{
	...
	if (!pInput) {
		LogMsg("Could not read the file %s for compression\n", fName.c_str());
		PressAnyKey();
		return true;
	}
	if (size >= C_RTFILE_PACKAGE_HEADER_BYTE_SIZE && IsAPackedFile(pInput))
	...
	LogMsg("Compressed (%.1f kb, %.0f%%%% smaller)", totalBytes / 1024.0f, 100.0f - 100.0f * float(totalBytes) / float(size));
#endif
	return true;
}
