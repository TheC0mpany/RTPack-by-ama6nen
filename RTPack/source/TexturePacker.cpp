...
void WriteTextureWithoutPVR(string pathAndFileName, SoftSurface* texture, int nNumMipLevels, bool bUsesTransparency, int originalWidth, int originalHeight, bool bFlipped, bool bAlphaFix)
{
	if (texture)
		texture->m_bFixAlpha = bAlphaFix;
	FILE* pFileOut = NULL;
	int nMipLevel;
	...
	if (bAlphaFix) {
		LogMsg("Packed with apc fix (%d x %d)", originalWidth, originalHeight);
	}else
			LogMsg("Packed (%d x %d) (Alpha premultiplication colors not fixed!)", originalWidth, originalHeight);
...
bool TexturePacker::ProcessTexture(string fName)
{
	...
	SoftSurface finalBuff;
	finalBuff.m_bFixAlpha = UseAlphaFix();
	...
	if (GetApp()->GetOutput() == App::JPG)
	{
		...
		string outputName = fileNameOnly + "_test.jpg";
	...
	*/
	...
	WriteTextureWithoutPVR(fileName, &finalBuff, nNumMipLevels, m_bUsesTransparency, originalX, originalY, bFlipped, m_bAlphaFix);
	return true;
